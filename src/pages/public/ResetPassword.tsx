...
import { ResetPasswordFormContext } from "@/types/form";
...
const ResetPassword = () => {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<ResetPasswordFormContext>({
    resolver: yupResolver(FormSchemas().ResetPasswordSchema),
  });

  const token = searchParams.get("token");

  const resetPassword = useMutation({
    mutationFn: (passwordCredentials: {
      password: string;
    ...
    onError: (error: any) => {
      toast.error(
        error?.response?.data?.message ??
          "Error encountered trying to reset your password."
      );
    },
    onSuccess: () => {
  ...
  const onSubmit: SubmitHandler<ResetPasswordFormContext> = async (data) => {
    console.log(data);

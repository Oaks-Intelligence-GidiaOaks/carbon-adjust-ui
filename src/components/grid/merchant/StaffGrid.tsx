...
const StaffGrid = ({ data }: { data: any[]; isUpdating: boolean }) => {
  ...
  const columns = [
    ...
    columnHelper.accessor((row: any) => row?.name, {
      ...
      header: () => <div className="w-36 text-left">Name</div>,
    }),
    ...
    columnHelper.accessor((row: any) => row?.email, {
      ...
      header: () => <div className="w-36 text-left">Email</div>,
    }),

    columnHelper.accessor((row: any) => row?.accessLevel, {
      id: "accessLevel",
      cell: (info) => (
        <div className="w-24 mx-auto text-left capitalize">
          {(info.getValue()[0].split("_").join(" ") as string).toLowerCase()}
        </div>
      ),
      header: () => <div className="w-36 text-left">Access Level</div>,
    }),
          ...
          <div className="relative px-4 z-10">
            ...
            {showModal && currentRowId === info.getValue() && (
              ...
              >
                ...
                >
                  <div className="rounded-full bg-ca-red p-1">
                    <XMarkIcon className="text-white text-base size-3" />
                  </div>

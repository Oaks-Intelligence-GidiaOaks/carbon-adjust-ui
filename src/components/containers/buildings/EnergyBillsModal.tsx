...
const EnergyBillsModal = ({ isOpen, closeModal, buildingId }: ModalProps) => {
  ...
  const deleteBillMutation = useMutation({
    mutationFn: (energyBillsId: number) => deleteEnergyBill(buildingId, energyBillsId.toString()),
    onSuccess: () => {
        ...
        <div className="bg-white rounded-md shadow-lg w-full max-w-3xl h-[80vh] p-6">
          ...
          <div className="mt-4 max-h-72 overflow-y-auto">
            {isLoading ? (
              ...
              <p className="text-red-500">Failed to load energy bills. Please try again.</p>
              ...
              <ul className="space-y-4">
                {energyBills.map((bill: EnergyBill, index: number) => (
                  <li key={bill._id} className="flex justify-between items-center border p-3 rounded-md">
                    <div>
                      <p className="text-sm font-medium break-all"> Bill {index + 1} - {bill._id}</p>
                    </div>
                    <div className="flex space-x-2">
                      <button onClick={() => setBillToView(bill)} className="text-blue-500 hover:text-blue-700 text-sm">
                        View
                      </button>
                      <button onClick={() => handleDeleteBill(bill._id)} className="text-red-500 hover:text-red-700">
                        <FiTrash2 size={18} />
                      </button>
                    </div>
                  </li>
          ...
          <div className="flex justify-end my-6">
            <button onClick={closeModal} className="py-2 px-4 bg-gray-500 text-white rounded-md hover:bg-gray-600">
              Close
            </button>
          </div>
        </div>
  ...
  return (
    <div className="fixed inset-0 flex bg-slate-500 bg-opacity-50 z-50 items-center justify-center">
      <div className="bg-white rounded-md shadow-lg w-full max-w-3xl h-full lg:h-[80vh] p-6 overflow-hidden">
        <div className="flex justify-between items-center border-b pb-3">
          <h2 className="text-lg font-semibold break-all">{bill._id} - View Document</h2>
          <button onClick={closeModal} className="text-gray-500">
            <AiOutlineClose size={20} />
          </button>
        </div>
...
const ConfirmationModal = ({ confirmDelete, cancelDelete, isMutating }: ConfirmationModalProps) => {
  return (
    <div className="fixed inset-0 flex bg-slate-500 bg-opacity-50 z-50 items-center justify-center">
      <div className="bg-white rounded-md shadow-lg w-full max-w-sm p-6">
        <h3 className="text-lg font-semibold">Confirm Deletion</h3>
        <p className="mt-2">Are you sure you want to delete this energy bill?</p>

        <div className="flex justify-end mt-6 space-x-2">
          <button onClick={cancelDelete} className="py-2 px-4 bg-gray-300 text-black rounded-md hover:bg-gray-400">
            Cancel
          </button>
          <button
            onClick={confirmDelete}
            className={`py-2 px-4 rounded-md flex justify-center items-center ${isMutating ? "bg-gray-300 cursor-not-allowed" : "bg-red-500 text-white hover:bg-red-600"}`}
            disabled={isMutating}
          >
            {isMutating ? (
              <>
